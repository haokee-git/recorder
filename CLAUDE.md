...
### 2026-02-12 - 提醒设置（声音与振动开关）

#### 需求背景
用户希望在设置中自定义提醒触发时的行为，可选是否播放声音、是否振动。

#### 具体需求

**1. 设置页新增"提醒设置"栏目**
- 位置：大模型 API 设置下方、界面设置上方
- 包含两个 Toggle 开关：
  - "提醒声音"：控制闹钟触发时是否播放声音（默认开启）
  - "提醒振动"：控制闹钟触发时是否振动（默认开启）
- 适配明暗主题

**2. 闹钟行为联动**
- AlarmActivity 根据设置决定是否播放感言音频
- AlarmReceiver 根据设置动态创建通知渠道（含/不含声音和振动）

#### 影响文件
- 修改 SettingsRepository.kt: 新增 alarm_sound / alarm_vibration 键
- 修改 SettingsViewModel.kt: 新增状态字段和 toggle 方法
- 修改 SettingsScreen.kt: 新增"提醒设置"栏目
- 修改 AlarmReceiver.kt: 根据设置动态配置通知渠道
- 修改 AlarmActivity.kt: 根据设置决定是否播放音频和振动
...
#### 需求背景
首次安装应用时展示引导页，介绍核心功能和截图，提升新用户的第一印象。

#### 具体需求

**1. 引导页展示**
- 7 张应用截图，每页上方显示图片，下方用简洁文艺的大字描述功能
- 底部左侧"跳过"按钮，右侧"继续"/"开始使用"按钮
- 按钮上方用圆点指示器显示页数，当前页圆点放大并染亮蓝色
- 仅首次启动显示，之后不再出现
...
#### 影响文件
- 重命名 7 张图片为合法 Android 资源名（onboarding_*.jpg）
- 新建 OnboardingScreen.kt: 引导页 UI
- 修改 SettingsRepository.kt: 新增 onboarding_completed 键
- 修改 MainActivity.kt: 首次启动判断和引导页显示
